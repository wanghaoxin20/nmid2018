<%@page import="biz.UserBizFactory"%>
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<sql:query var="blog_info" dataSource="${blog}">
									select * from blog_info where id in (select blog_id from userblog where username='${user.username}')
									 and (title like '%${param.keyword}%' or id in (SELECT blog_id from blog_text where text like '%${param.keyword}%'))
								</sql:query>
								<%/*******************/ %>
								...
								<c:forEach items="${blog_info.rows}" var="row">
									...
									<tr>
										...
										<td><a href="DelBlog?id=${row.id}"
											class="ico del">删除</a><a
											href="ShowBlog?opt=update&id=${row.id}"
